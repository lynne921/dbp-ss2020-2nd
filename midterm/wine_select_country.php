<?php
  ...
  if (isset($_GET['country'])) {
    ...
    $query = "SELECT * FROM wine_all WHERE country = '{$country}'"; } 
...
<body>
    ...
    <form action = "wine_select_country.php" method = "GET">
        <select name="country">   
            <?php
                    $sql = "SELECT DISTINCT country FROM wine_all
                    ORDER BY country;";

                    $result = mysqli_query($link, $sql);
                    while ($result_array = mysqli_fetch_array($result)){
                        print "<option value='".$result_array['country']."' ";
                        print ">".$result_array['country']."</option>\n";
                    }
            ?>
        </select>
